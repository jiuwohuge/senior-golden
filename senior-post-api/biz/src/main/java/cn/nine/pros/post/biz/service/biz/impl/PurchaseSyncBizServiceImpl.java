...
public class PurchaseSyncBizServiceImpl implements PurchaseSyncBizService {
    ...
    /**
     * PENDING hard-clear: expire this token VIP, expire all ACTIVE VIP for user,
     * revoke entitlement by code, syncVipEntitlement(false). Do NOT re-grant from remaining VIP.
     */
    private void handlePendingEntitlementAndVip(
        ...
        vipSubscriptionService.expireActiveForUser(userId, userId);
        userEntitlementService.revokeEntitlementByCode(userId, entitlementCode, userId);
        LocalDateTime clearedEnd = now.minusMinutes(1);
        userService.syncVipEntitlement(userId, false, clearedEnd, userId);
        log.info("entitlement hard-cleared for pending purchase, userId={}, code={}", userId, entitlementCode);
    }

    /** Expire VIP row for purchaseToken if present; do not create a row for bare PENDING. */
