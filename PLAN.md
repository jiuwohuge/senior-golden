...
- **本次新增（2026-05-23，登录页窄屏无需滚动显示主按钮优化）**：
  ...
  - 实际处理：
    - `senior-post-flutter/lib/features/auth/login_page.dart`
      - 引入窄屏自适应（`height < 860`）并收紧关键垂直间距。
      - 登录页协议勾选改为紧凑模式，减少三行文案占高。
    - `senior-post-flutter/lib/widgets/postal/postal_checkbox_field.dart`
      - 新增 `compact` 参数：缩小勾选框、行高与内边距，保留可读性与链接可点性。
  - 验证：
    - `flutter analyze` 通过。

- **本次新增（2026-05-23，注册同意条款位置调整）**：
  - 目标：将“同意隐私政策/用户协议”从头像步骤移至最终确认步骤，流程语义更清晰。
  - 实际处理：
    - `senior-post-flutter/lib/features/auth/register_page.dart`
      - 头像步骤移除协议勾选，仅保留头像选择/跳过提示；
      - 最后确认步骤新增协议勾选与条款链接；
      - 校验改为仅在最终提交时强制要求勾选；
      - 取消从头像步骤提前注册账号的逻辑，改为最终提交时统一注册。
  - 验证：
    - `flutter analyze` 通过。
