...
> **版本**：4.0 · **基线分支**：`master`（原 feature_4.0 工程基线已合入） · **更新**：2026-07-14 · **维护**：项目 Owner + AI 架构师  
> **需求真源**：[`docs/eng/4.0需求改版.md`](4.0需求改版.md)（PRD v4.0，§1~§20）  
> **后端框架能力真源**：[`senior-post-api/底层框架能力.md`](../../senior-post-api/底层框架能力.md)（索引入口 [`底层框架能力.md`](底层框架能力.md)）  
> **治理**：旧版 `doc/plan/**`、`doc/requirements/**`、根目录 `findings.md/progress.md/task_plan.md` 已废弃删除；本文件为 4.0 工程规划基线。  
>  
> **⚠️ 产品改版（2026-08）**：定位转向「**时光邮局 · 时光信主路径**」，决策真源见 [`docs/decisions/改版方向讨论稿.md`](../decisions/改版方向讨论稿.md)。与本文「笔友/匹配主叙事」冲突时，**产品决策以讨论稿为准**；本文仍描述已交付的 M0–M7 工程事实。产品讨论与整改计划见 [`docs/product/改版整改落地计划.md`](../product/改版整改落地计划.md)；实现按上架最小验收排期。
...
- [x] 行为事件采集(§14)；匹配 v1 规则版(§7)
- [x] POST_OFFICE 端到端；内容审核(§15.6)；风控额度/保护池(§15)

### M4 — 关系 + 笔友页 + 推荐 + 个人中心 + 信箱（约 3~4 周）
- [x] 关系极简模型(§10)；笔友页三 Tab(§8/§12.3)；推荐(§9)
- [x] 个人中心(§13)；信箱流水(§12.4)

### M5 — 仪式 + 商业 + 配置 + 推送 + 匹配 v2（4 周+）
- [x] 商业/管理后台配置(§16/§20.2)；推送(§11.6)；匹配 v2 AI(§7.3) — **骨架已交付**
- [~] 仪式体验(§5.4) — 邮戳/开信 overlay 已有，在途进度/读信皮肤/写信模板见 **M6**
- [ ] **上线前**：Flyway 迁移链统一整理一次（§1.3），再冻结生产策略

### M6 — 体验闭环补齐（遗漏收口）
- [x] 废弃产品面平邮/挂号；发信固定 STANDARD，速度仅 §6.1
- [x] 注册定位：lat/lng 反查 + IP/手动降级(§3.3)
- [x] 目录/推荐全路径不可见自己
- [x] 新手引导首封信(§2.8) + 首封匹配加权(§7.5)
- [x] 每日免费额度强制领取弹窗（不可关闭）
- [x] §11.4 在途明细页 + 相对 ETA/进度；首页摘要卡跳转修正
