---
name: M7 管理后台运营
overview: 按你确认的范围（业务主体 B：用户/信件/笔友/商业；邮件追踪 C：系统邮件 + 用户信件），在现有 React+Ant Design Manage 与 `/webapi` 五层架构上交付 M7：统一列表规范、补齐运营页、管理员操作日志与增强看板，并回写 PLAN.md / PRD。明确不做任何导入/导出。
todos:
  - id: m7-table-kit
    content: Manage 公共 AdminProTable / EnumSelect（无导出工具）
    status: completed
  - id: m7-user-ops
    content: 用户多维查询+枚举下拉+分页排序批量（无导入导出）
    status: completed
  - id: m7-letter-ops
    content: 信件运营页（审核接线+状态追踪+详情，无导出）
    status: completed
  - id: m7-mail-outbox
    content: 系统邮件 outbox 分页/预览/重试 Manage+API
    status: completed
  - id: m7-penpal-commerce
    content: 笔友关系运营页 + 商业商品列表规范强化
    status: completed
  - id: m7-oplog-dash
    content: 管理员操作日志接线 + 看板指标增强
    status: completed
  - id: m7-docs
    content: 回写 PLAN.md M7 + PRD §21/§20.3 + 验收
    status: completed
isProject: false
---
...
| 不做   | App 端改版；Flyway 迁移链合并（仍属上线前窗口）；真实 IAP；**任何导入/导出（CSV/Excel/批量文件均不做）** |
...
- **缺口**：普通信件审核 API（`[AdminLetterAuditApi](senior-post-api/client/src/main/java/cn/nine/pros/post/client/api/admin/AdminLetterAuditApi.java)`）**无 Manage 页面**；无笔友运营页；无 `sys_mail_outbox` 查询；用户国家等仍手输；`log_admin_operation` 表存在但**无写入/无 `/webapi`**；列表普遍缺真实分页/排序/批量
- **明确不做**：导入/导出（与产品决策一致，本里程碑不交付、不预留半成品按钮）
...
**前端**：真实分页；多维筛选；枚举全 Select。无导入/导出入口。
...
- 真实分页/排序；按 type/status/price 筛选
- 批量上下架（status）
...
- 运维动作：失败重试（重置 status/next_retry_at）— 纳入本里程碑
...
- 在 Biz 关键写路径埋点：用户改状态/删、信件审核、笔友解除、商品上下架、邮件重试
...
1. 更新 `[PLAN.md](PLAN.md)`：新增 **M7 — 管理后台运营** checklist；决策记录写入范围 B+C，并写明 **不做导入/导出**
2. 更新 `[doc/4.0需求改版.md](doc/4.0需求改版.md)`：新增 **§21 管理后台运营**（或 §20.3）：主体列表、双轨邮件、操作日志、看板指标；**不写导入导出能力**
...
- 冒烟：用户多维筛选→编辑下拉；信件审核通过/拒绝；出站邮件失败重试；笔友列表；看板新指标；操作日志有写入
- 确认全站无导入/导出入口与 API
