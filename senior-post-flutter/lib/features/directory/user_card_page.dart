...
class _UserCardPageState extends ConsumerState<UserCardPage> {
  ...
  bool _penpalRequestBusy = false;

  Future<void> _sendPenpalRequest(BuildContext context, AppUser user) async {
    if (_penpalRequestBusy) return;
    setState(() => _penpalRequestBusy = true);
    final l10n = AppLocalizations.of(context)!;
    try {
      await ref
          .read(relationRemoteProvider)
          .createPenpalRequest(peerUserId: user.id);
      ref.invalidate(relationWithProvider(userId));
      ...
      if (mounted) setState(() => _penpalRequestBusy = false);
    }
  }

  void _openCompose(BuildContext context, AppUser user) {
    context.push(
      '/compose',
      extra: ComposeIntent(
  ...
  String _fabLabel(AppLocalizations l10n, RelationSnapshot? relation) {
    final state = relation?.displayState;
    if (state == RelationDisplayState.stranger) {
      return l10n.userCardWriteFirstLetter;
    }
    if (state == RelationDisplayState.canAddPenpal && relation!.canAddPenpal) {
      return l10n.relationAddPenpal;
    }
    if (state == RelationDisplayState.contacting ||
        state == RelationDisplayState.pendingOut ||
        state == RelationDisplayState.pendingIn) {
      return l10n.userCardContinueWriting;
    }
    return l10n.userCardSendLetter;
  }
  ...
  VoidCallback? _fabAction(
    ...
    final state = relation?.displayState;
    if (state == RelationDisplayState.canAddPenpal && relation!.canAddPenpal) {
      return _penpalRequestBusy
          ? null
          : () => _sendPenpalRequest(context, user);
    }
    return () => _openCompose(context, user);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return async.when(
      ...
      data: (user) {
        ...
        final relationAsync = showFab
            ? ref.watch(relationWithProvider(userId))
            : const AsyncValue<RelationSnapshot?>.data(null);
        final relation = relationAsync.valueOrNull;
        final isPenpal =
            relation?.penpal == true ||
            user.relationDisplayState == RelationDisplayState.penpal ||
            user.postalFriend;

        return Scaffold(
          backgroundColor: PostalTokens.paperCream,
          appBar: _appBar(context, l10n, showFab, user, isPenpal),
          floatingActionButton: showFab
              ? FloatingActionButton.extended(
                  ...
                  foregroundColor: Colors.white,
                  icon: Icon(
                    relation?.displayState ==
                                RelationDisplayState.canAddPenpal &&
                            relation!.canAddPenpal
                        ? Icons.person_add_outlined
                        : Icons.mail_outline_rounded,
                    size: 22,
                  ),
                  label: Text(
                    _fabLabel(l10n, relation),
                    style: const TextStyle(
                      fontWeight: FontWeight.w800,
                      letterSpacing: 0.25,
                    ),
                  ),
                  onPressed: _fabAction(context, user, relation),
                )
              : null,
                ...
                PostalCardEnvelope(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (relation?.displayState != null) ...[
                        const SizedBox(height: 10),
                        PostalStatusChip.draft(
                          label: relationDisplayLabel(
                            l10n,
                            relation!.displayState,
                          ),
                        ),
