...
| 设备标识与挤下线 | 进行中（**FP-A1-006**） | 设备与 `user_device` 一致，支撑单端策略 | 请求头/体与落库路径核对 |
...
| **运营公告（可关闭，非强更）** | 未开始（**FP-X-003**） | 固定版式：标题、版本号、更新说明（纯文本多行） | 见 `findings.md` §18 |
...
| 邮件模板多语言 | 未开始（**FP-A10-001**） | 发信侧按 Locale 渲染 subject/body | 依赖 **FP-X-001**；`MessageSource` 键 |
...
| 功能 | 状态 | 预期行为 | 验收标准 |
|------|------|----------|----------|
| 邮件服务 + Outbox | 未开始（**FP-X-001**） | 异步发信可重试 | 本地/测试环境收到邮件 |
| OSS 预签名 | 已完成 | 各 scene 可上传 | 配置 `senior-post.oss` 后可用 |
| **运营公告（可关闭，非强更）** | 未开始（**FP-X-003**） | 固定版式：标题、版本号、更新说明（纯文本多行）；Manage 表单 + App 模板 | 见 `findings.md` §18 |
...
| 模块 | 已完成（概览） | 主要缺口 |
|------|----------------|----------|
| 1–2 | 注册登录、bootstrap、资料 PATCH、忘记密码、兴趣、头像 | **FP-A1-006** 设备落库、**FP-A1-007** AES、冷启动拉 `me` |
...
| 9–11 | Manage 主体、注销 MVP、i18n 基线 | **FP-X-001**、**FP-X-003**、**FP-A10-001** |
